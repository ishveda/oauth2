...
func TestRetrieveTokenBustedNoSecret(t *testing.T) {
	...
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, nil, url.Values{}, http.Header{})
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	cancel()
	_, err = RetrieveToken(ctx, clientID, "", cancellingts.URL, nil, url.Values{}, http.Header{})
	close(retrieved)
	if err == nil {
